    ...
    F: CanFrame<Channel = C> + Clone + Send + Display + 'static,
{
    ...
    #[inline(always)]
    pub fn byte_order(&self, channel: C) -> Option<ByteOrder> {
        self.context.get(&channel).map(|v| v.byte_order)
    }

    #[inline(always)]
    async fn with_context<R, Func, Fut>(&self, channel: C, callback: Func) -> Result<R, DoCanError>
    where
        Func: Fn(IsoTpListener, CanIsoTp<C, F>, DidConfig) -> Fut + Send,
        Fut: std::future::Future<Output = Result<R, DoCanError>> + Send,
    {
        match self.context.get(&channel) {
            Some(ctx) => {
                let guard = ctx.did.lock().await;
                let did = guard.clone();
                drop(guard);
                callback(ctx.listener.clone(), ctx.iso_tp.clone(), did).await
            }
    ...
    #[inline(always)]
    async fn with_security_algo_ctx<R, Func, Fut>(
        &self,
        channel: C,
        callback: Func,
    ) -> Result<R, DoCanError>
    where
        Func: Fn(IsoTpListener, CanIsoTp<C, F>, DidConfig, SecurityAlgo) -> Fut + Send,
        Fut: std::future::Future<Output = Result<R, DoCanError>> + Send,
    {
        match self.context.get(&channel) {
            Some(ctx) => {
                ...
                let did = guard.clone();
                drop(guard);
                let algo = ctx.security_algo.lock().await.clone();
                match algo {
                    Some(algo) => {
                        callback(ctx.listener.clone(), ctx.iso_tp.clone(), did, algo).await
                    }
                    None => Err(DoCanError::OtherError("security algorithm required".into())),
                }
            }
            None => Err(DoCanError::OtherError(format!(
                "channel: {} is not initialized",
    ...
    #[inline(always)]
    async fn context_util<R, Func, Fut>(&self, channel: C, callback: Func) -> Result<R, DoCanError>
    where
    ...
    async fn suppress_positive_sr(
        ...
        match Self::send_and_response(listener, iso_tp, did, addr_type, request).await {
            Ok(r) => Ok(Some(r)),
            Err(e) => match e {
    ...
    async fn send_and_response(
        ...
        let mut response = Response::try_from((data, did)).map_err(DoCanError::ISO14229Error)?;
        while Self::response_service_check(&response, service)? {
            ...
            let (_, request) =
                Self::tester_present_request(&did, TesterPresentType::Zero, true).await?;
            iso_tp
                .write(addr_type, request.into())
                .await
                .map_err(DoCanError::IsoTpError)?;
            ...
            response = Response::try_from((data, did)).map_err(DoCanError::ISO14229Error)?;
        }
